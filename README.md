...
This line also closes a silent-loss bug in the 2.4.x line. With `analysis_server_plugin 0.3.15` the plugin reported *idle* to the analysis server between its first file and its full pass, so `dart analyze` (which stops collecting at the first idle status) returned with zero or partial plugin diagnostics and exit code 0: on this package's fixtures 11 of 20 runs (2-row fixture) and 7 of 10 runs (16-row fixture) lost rows, and 15 of 15 did under parallel load. `analysis_server_plugin 0.3.16` reworked the plugin server so its status follows the analysis driver; on 0.3.22 the same fixtures delivered every row in 55 of 55 `dart analyze` runs, including 15 under the same load. See `tools/lint_sentinel/README.md` for the measurements and a CI guard that still verifies delivery.
