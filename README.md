...
**Riverpod State After Async Gap** (`riverpod_state_after_async_gap`) - Reports an unguarded `state = …` write or `state` read after an `await` in provider methods, including `state = await …` and same-statement reads that run after an `await` (`await foo() ?? state`). Receiver-first forms (`state.foo(await x)`) and locals/parameters named `state` are not reported; `this.state` still is. Riverpod 3 throws `UnmountedRefException` from both the `state` setter and getter once the provider is disposed, so the guard belongs right after the await: `await …; if (!ref.mounted) return;` (or `final next = await …; if (!ref.mounted) return; state = next;`). **Disabled by default** because the `state = await` idiom is common in existing apps; enable it per project:
