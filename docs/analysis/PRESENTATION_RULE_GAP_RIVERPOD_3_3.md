...
| `riverpod_ref_after_async_gap` | INFO | **fixed (state write)** | Path-sensitive `ref.*` + unguarded `state =` / `this.state =` after await; `ref.mounted` guards shared | `riverpod_ref_after_async_gap_rule.dart`; `riverpod_ref_after_async_gap_v2_rule_test.dart` |
...
3. **`state =` after dispose still throws** in Riverpod 3 — tracked as INFO by
   `riverpod_ref_after_async_gap` when unguarded after an async gap.
