...
독자 기준 (cold reader) — 리뷰어가 결함을 찾게 돕는 본문의 전제:
- 이 PR만 읽는 사람이 이해할 수 있어야 합니다. 작업 세션·계획 문서·이전 PR 맥락을 아는 독자를 가정하지 않습니다.
- 내부 코드네임·계획 명칭·축약어는 첫 등장에서 한 줄로 풀어 쓰거나 정의 문서를 링크합니다.
- 제목과 요약은 완전한 문장으로 씁니다. 명사만 나열한 제목을 금지합니다. 제목에는 "무엇을 한다"가 드러나는 동사가 들어갑니다.
-->

## 🌷 요약

<!-- 이 PR이 무엇을 바꾸는지 완전한 문장 1~3개로 적습니다. 내부 용어는 여기서 풀어 씁니다.
     기준은 lint 통과가 아니라 "처음 보는 사람이 10초 안에 이해"입니다. -->
...
<!-- 이 PR이 왜 필요한지, 사용자/운영/제품에 어떤 변화가 생기는지 적습니다.
     고려했지만 선택하지 않은 접근이 있으면 한 줄로 남깁니다 — 리뷰어가 설계를 검증하는 출발점입니다. -->

-

## 🔧 변경 사항

<!-- 각 bullet은 리뷰어가 diff와 대조해 참/거짓을 판정할 수 있는 "동작 주장"으로 씁니다.
     좋은 예: "`merge_pr`가 병합에 성공하면 `auto-merged` 라벨을 붙인다"
     나쁜 예: "`pr-autopilot.sh` 수정" (파일명 나열 — 무엇이 달라졌는지 검증 불가) -->
...
<!-- "검증하지 않은 영역"은 리뷰어가 가장 집중해서 봐야 할 곳입니다. 비워두지 말고 구체적으로 적습니다. -->
...
<!-- 에이전트는 위험을 낮게 보더라도 근거를 적습니다.
     "특히 봐야 할 점"에는 작성자가 가장 자신 없는 결정·경계 조건을 파일:라인 수준으로 적습니다.
     "읽는 순서"는 변경 파일 3개 이상이면 리뷰 순서·파일 역할을 한 줄씩, 2개 이하면 "해당 없음"(또는 선택적 한 줄)으로 적습니다. -->
...
- [ ] PR title이 `<type>: ITT-123 한국어 요약` 또는 `<type>: 한국어 요약` 형식이고, 동사가 들어간 완전한 요약이다.
- [ ] 사용자에게 보이는 PR 본문은 한국어로 작성했다.
- [ ] 내부 코드네임·축약어를 본문에서 풀어 썼거나 정의 문서를 링크했다 (cold reader 기준).
- [ ] 변경 사항 bullet이 파일명 나열이 아니라 검증 가능한 동작 주장이다.
