...
''',
    ...
    test(
      'reports ref access inside Stream.listen, Timer, and addListener callbacks',
      () async {
        final result = await V2RuleHarness(rule: RiverpodRefAfterAsyncGapRule())
            .analyze(
              files: {
...
@riverpod
class TodoNotifier {
  void subscribe(Stream<int> todos) {
    todos.listen((_) {
      ref.read(todoProvider);
    });

    todos.listen((_) {}, onError: (error) {
      ref.invalidate(todoProvider);
    }, onDone: () {
      ref.refresh(todoProvider);
    });
...
''',
        ...
        result.expectDiagnostics([
          ...
          const ExpectedV2Diagnostic(
            ...
            codeName: 'riverpod_ref_after_async_gap',
            problemMessage:
                'Avoid ref.watch() after an async gap in Riverpod providers.',
            correctionMessage:
                'Capture provider/usecase dependencies before the async gap, or guard the post-gap access with "if (!ref.mounted) return;".',
          ),
          const ExpectedV2Diagnostic(
    ...
    test(
      'does not report ref.listen callbacks or mounted-guarded stream callbacks',
      () async {
        final result = await V2RuleHarness(rule: RiverpodRefAfterAsyncGapRule())
            .analyze(
              files: {
...
''',
    ...
    test(
      'reports ref access inside a parenthesized async Stream.listen callback',
      () async {
        final result = await V2RuleHarness(rule: RiverpodRefAfterAsyncGapRule())
            .analyze(
              files: {
...
''',
    ...
    test(
      'reports ref access inside Timer.run and Timer.periodic without new',
      () async {
        final result = await V2RuleHarness(rule: RiverpodRefAfterAsyncGapRule())
            .analyze(
              files: {
...
''',
    ...
    test(
      'honours a ref.mounted guard inside an unbraced case after an outer await',
      () async {
        // The guard recognition in _isDisposalGuarded walks SwitchMember
        // statements like Block statements; it can only suppress a report,
        // so the unguarded sibling case must still be reported.
        final result = await V2RuleHarness(rule: RiverpodRefAfterAsyncGapRule())
            .analyze(
              files: {
                'lib/features/todo/presentation/providers/todo_notifier.dart':
