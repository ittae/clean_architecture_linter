...
class BusinessLogicIsolationRule extends DartLintRule {
  ...
  static const _code = LintCode(
    ...
    correctionMessage: 'Move business logic to UseCases, Entities, or domain services. Keep UI components focused on presentation.',
  );
  ...
  void _checkDirectBusinessLogicCalls(
    ...
    if (_isRepositoryCall(methodName, target)) {
      final code = LintCode(
        ...
        correctionMessage: 'Use UseCases or state management to handle data operations instead of direct repository calls.',
      );
    ...
    if (type is NamedType) {
      ...
      if (_isRepositoryType(typeName)) {
        final code = LintCode(
          ...
          correctionMessage: 'Remove direct repository dependencies. Use state management or dependency injection patterns.',
        );
      ...
      if (_isDomainServiceType(typeName)) {
        final code = LintCode(
          ...
          correctionMessage: 'UI components should not depend directly on domain services. Use proper architectural patterns.',
        );
        reporter.atNode(type, code);
      }
    }
  ...
  bool _isRepositoryCall(String methodName, Expression? target) {
    ...
    final repositoryMethods = [
      'save', 'create', 'update', 'delete', 'find', 'get',
      'fetch', 'load', 'store', 'persist', 'remove'
    ];

    return repositoryMethods.any((method) => methodName.toLowerCase().contains(method)) ||
  ...
  bool _isUseCaseCall(String methodName, Expression? target) {
    return methodName == 'call' || methodName == 'execute' ||
  ...
  bool _isDomainServiceType(String typeName) {
    final domainServicePatterns = [
      'Service', 'UseCase', 'DomainService', 'BusinessService'
    ];
  ...
  BusinessLogicComplexity _calculateBusinessLogicComplexity(BlockFunctionBody body) {
    ...
    return BusinessLogicComplexity(
      ...
      hasComplexBusinessLogic: visitor.ifCount > 3 ||
                               visitor.loopCount > 1 ||
...
class _CalculationLogicVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    final calculationMethods = [
      'sum', 'total', 'average', 'calculate', 'compute'
    ];
...
class _DataTransformationVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    final transformationMethods = [
      'map', 'transform', 'convert', 'parse', 'format', 'serialize'
    ];
