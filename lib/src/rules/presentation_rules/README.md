...
- ❌ an unguarded call to a sync private method that reads or writes `state` in its own body (one hop). The finding is the call. A method that only calls another private method is not indexed, so `await fetch(); _outer();` stays quiet when only `_inner()` writes `state`. State access that exists only inside a nested function does not make the call a finding; a call to an async private method is not a finding at the call site
